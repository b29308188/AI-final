...
def nearestMarker(pusher):
    ...
    minDistM = mList[0]
    for m in mList:
        distPM = vecSub(pusher.pos, m.pos).mag()
        if distPM < minDist:
            minDist = distPM
            minDistM = m
...
def nearestNotRedRegion(pos):
    ...
    for i in range( len( regionList ) ):
        for j in range(len(regionList[i])):
    ...
    for i in range(len(vertexList)):
        if (vertexColors[ i ] & 0x1 ) == 1 and vertexColors[ i ] != 1 :
            candidates.append( i )
          
    minDist = float("inf")
    minDistVertice = vertexList2D[0]
    for i in range(len(candidates)):
        dist = vecSub(pos, vertexList2D[candidates[i]]).mag()
        if dist < minDist:
